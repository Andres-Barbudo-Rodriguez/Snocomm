...
public void do_file(const char *name)
{
  ...
  switch (f.st.st_mode & S_IFMT)
    ...
    case S_IFCHR: /* char dev */
      prompt_unlink(name, "character device file", W_OK, f.st.st_mode);
      return;
...
/*
  fsetbuf -- inits fsize, bufsize, and loop
*/
...
public void fsetbuf(struct file_s *f)
{
  ...
  */
  ...
  /* 
     ...
     do ++f->fsize; while (f->fsize % f->st.st_blksize);

     the above is the old unbounded loop version. the new
     version below has a deterministic number of ops.
     not a big deal, obviously, but what the hell, it looks
     neater.

     thanks to my brother, david, for rediscovering
     this little trick that's also in write3_pass()
  */
...
#ifdef SANITY
  if ((unsigned long int) f->buf % sizeof(urand_t))
    {
      fprintf(stderr, "\r%s: buffer not %ld byte aligned! Skipping `%s'\n",
	      argvzero, (long int) sizeof(urand_t), f->name);
      return FAILED;
    }
#endif
