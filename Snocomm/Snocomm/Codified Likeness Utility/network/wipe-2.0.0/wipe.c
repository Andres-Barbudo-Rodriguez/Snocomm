...
{
#ifdef PAUSE
  /* this is the best place, since percent_update is always called */
  printf("Hit a key:");
  fgetc(stdin);
#endif
}
...
private int zero(struct file_s *f)
{
  ...
  if (write_pass(f, 0)) return FAILED;
...
public int wipe_passes(struct file_s *f)
{
  ...
  loop = options.wipe_multiply + 1;

  while (loop--)
    {
      if (options.random)
	if (random_pass(f, options.random_loop+1))
	  return FAILED;
